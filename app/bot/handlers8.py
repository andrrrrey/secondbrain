...
from app.config.settings import runtime_allowed_ids, settings
...
async def handle_text(message: Message, state: FSMContext) -> None:
    ...
    # If in admin flow — skip (handled by admin FSM handlers)
    if current in (AdminStates.waiting_add_user, AdminStates.waiting_api_key):
        return
...
async def cb_admin_list(callback: CallbackQuery) -> None:
    ...
    await callback.answer()
    if not runtime_allowed_ids:
        text = "Список пуст — доступ открыт всем."
    else:
        lines = []
        for uid in sorted(runtime_allowed_ids):
...
async def cb_admin_remove(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    removable = sorted(uid for uid in runtime_allowed_ids if uid != settings.admin_user_id)
    if not removable:
        await callback.message.edit_text(
...
async def cb_admin_rm(callback: CallbackQuery) -> None:
    ...
    runtime_allowed_ids.discard(uid)
...
async def cmd_api(message: Message, state: FSMContext) -> None:
    ...
    current_masked = cfg.runtime_api_key[:8] + "..." + cfg.runtime_api_key[-4:]
...
async def handle_api_key(message: Message, state: FSMContext) -> None:
    if not _is_admin(message.from_user.id):
        return
