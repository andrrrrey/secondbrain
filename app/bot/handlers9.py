...
async def cmd_admin(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    count = len(rcfg.runtime_allowed_ids)
    await message.answer(
...
async def cb_admin_list(callback: CallbackQuery) -> None:
    ...
    await callback.answer()
    if not rcfg.runtime_allowed_ids:
        text = "Список пуст — доступ открыт всем."
    else:
        lines = []
        for uid in sorted(rcfg.runtime_allowed_ids):
...
async def cb_admin_remove(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    removable = sorted(uid for uid in rcfg.runtime_allowed_ids if uid != settings.admin_user_id)
    if not removable:
        await callback.message.edit_text(
...
async def cb_admin_rm(callback: CallbackQuery) -> None:
    ...
    rcfg.runtime_allowed_ids.discard(uid)
...
async def handle_add_user(message: Message, state: FSMContext) -> None:
    ...
    rcfg.runtime_allowed_ids.add(uid)
    await state.clear()
    await message.answer(
...
async def cmd_api(message: Message, state: FSMContext) -> None:
    ...
    current_masked = rcfg.runtime_api_key[:8] + "..." + rcfg.runtime_api_key[-4:]
